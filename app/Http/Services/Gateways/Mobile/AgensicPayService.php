...
class AgensicPayService
{
    protected $agensicService;

    public function __construct(AgensicService $agensicService)
    {
    ...
    {
        $country=strtoupper($transaction->beneficiary->country)=='CG'?'Congo':'DRC';
        $payout = $this->agensicService->withdraw([
            'country' => $country,
            'phone'        => $transaction->beneficiary->mobile_wallet,
            'amount'       => $transaction->amount,
            'carrier'       => $this->matchOperator($transaction->beneficiary->country, $operatorCode),
        ]);

        if ($payout['status']==false){
            // Optionnel : Update le statut en 'failed' ici si le provider rejette immédiatement
            throw new Exception("AgensicPay: " . $payout['message'], 422);
        }

        $transaction->update(['provider' => 'agensicpay']);
    ...
    private function matchOperator(?string $country_code, ?string $operator_code): ?string
    {
        ...
        return strtolower(trim($operator_code));
    }
}
